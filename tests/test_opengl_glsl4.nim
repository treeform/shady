when defined(shadyRunOpenGL):
  ...
  proc compileProgram(vertexSource, fragmentSource: string): GLuint =
    let vertexShader = compileStage(GL_VERTEX_SHADER, vertexSource, "glsl4.vert")
    let fragmentShader = compileStage(GL_FRAGMENT_SHADER, fragmentSource, "glsl4.frag")
    ...
    if ok == 0:
      raise newException(
        Exception,
        "GLSL4 program failed:\n" &
          getErrorLog(result, "glsl4.program", glGetProgramiv, glGetProgramInfoLog)
      )
  ...
  let program = compileProgram(
    toShader(layoutVertex, glsl4Desktop, shaderVertex),
    toShader(texturedFragment, glsl4Desktop, shaderFragment)
  )
  ...
  echo "OpenGL GLSL4 execution test passed"
else:
  echo "OpenGL GLSL4 execution test skipped; run with -d:shadyRunOpenGL"
